...
def scan_demand(
    ...
    validate_config(config)
    scan_dir = Path(output_root) / datetime.now().strftime("%Y%m%d_%H%M%S")
    scan_dir.mkdir(parents=True, exist_ok=True)
    index_rows: list[dict[str, object]] = []
    summaries: list[EpisodeSummary] = []

    for demand, controller, seed in planned_runs(config, controllers):
        result = run_experiment(config, demand, controller, seed, output_root=scan_dir / "runs", use_gui=use_gui)
        index_rows.append(
            {
