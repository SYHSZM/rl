...
def write_report_if_possible() -> None:
    required = {
        ...
        "pytest_guard": AUDIT_DIR / "pytest_real_workspace_hash_guard.json",
        "pytest_copy_manifest": AUDIT_DIR / "pytest_sandbox" / "copy_manifest.json",
        "nonisolated_incident": AUDIT_DIR / "pytest_nonisolated_incident" / "incident_record.json",
    }
    ...
    pytest_guard = json.loads(required["pytest_guard"].read_text(encoding="utf-8"))
    pytest_copy_manifest = json.loads(required["pytest_copy_manifest"].read_text(encoding="utf-8"))
    nonisolated_incident = json.loads(required["nonisolated_incident"].read_text(encoding="utf-8"))
...
- 全量 pytest 从本轮起只在 `pytest_sandbox/project/` 的精简隔离副本运行，`--basetemp` 也位于 `pytest_sandbox/tmp/`。复制范围为8个所需 production Python modules、全部现有 tests 和5个 network source XML，不复制 outputs 或大文档；{len(pytest_copy_manifest["files"])} 个源/目标文件的 SHA-256 见 `pytest_sandbox/copy_manifest.json`。
- pytest 前后分别哈希真实工作区 tracked 文件及全部关键 `network/*.xml`；`pytest_real_workspace_hash_guard.json` 记录 tracked_unchanged={pytest_guard["tracked_unchanged"]}、critical_network_xml_unchanged={pytest_guard["critical_network_xml_unchanged"]}、unchanged={pytest_guard["unchanged"]}。
...
- pytest 隔离 cwd：`{json.loads(pytest_result_path.read_text(encoding="utf-8")).get("isolated_cwd", "NA")}`；真实工作区测试前后哈希 unchanged={pytest_guard["unchanged"]}。
...
def preserve_nonisolated_pytest_incident() -> None:
    incident_dir = AUDIT_DIR / "pytest_nonisolated_incident"
    incident_dir.mkdir(parents=True, exist_ok=True)
    archived = []
    for name in ("pytest_stdout.log", "pytest_stderr.log", "pytest_junit.xml", "pytest_result.json"):
        source = AUDIT_DIR / name
        target = incident_dir / name
        if source.exists() and not target.exists():
            shutil.copy2(source, target)
        if target.exists():
            archived.append({"path": name, "bytes": target.stat().st_size, "sha256": sha256(target)})
    record_path = incident_dir / "incident_record.json"
    if not record_path.exists():
        record = {
            "incident": "fix round 1 ran pytest with cwd=PROJECT_ROOT; the existing integration test invoked unpatched env.run_experiment and rewrote eight pre-existing network/det_*.xml files plus network/merge.net.xml",
            "baseline_limitation": "original bytes were not backed up, so restoration cannot be proven; no deletion or guessed restoration is attempted",
            "tracked_production_code_diff": "none observed",
            "archived_files": archived,
        }
        record_path.write_text(json.dumps(record, indent=2, sort_keys=True), encoding="utf-8")


def real_workspace_snapshot() -> dict[str, Any]:
    git_base = ["git", "-c", "safe.directory=D:/python_install/py"]
    repo_result = subprocess.run(
        git_base + ["-C", str(PROJECT_ROOT), "rev-parse", "--show-toplevel"],
        text=True,
        encoding="utf-8",
    ...
    tracked_result = subprocess.run(
        git_base + ["-C", str(repo_root), "ls-files", "-z"],
        text=True,
        encoding="utf-8",
        errors="surrogateescape",
        capture_output=True,
    ...
    for relative in tracked_result.stdout.split("\0"):
        if not relative:
            continue
        path = (repo_root / relative).resolve()
        try:
            project_relative = path.relative_to(PROJECT_ROOT.resolve())
        except ValueError:
            continue
        if path.is_file() and AUDIT_DIR.resolve() not in path.parents:
            tracked[str(project_relative).replace("\\", "/")] = sha256(path)
    critical_network = {
    ...
    return {
        ...
        "tracked": tracked,
        "critical_network_xml": critical_network,
    }


def build_pytest_sandbox() -> Path:
    sandbox_root = AUDIT_DIR / "pytest_sandbox"
    ...
    network_names = ["merge.add.xml", "merge.con.xml", "merge.edg.xml", "merge.nod.xml", "merge.tll.xml"]
    sources = [PROJECT_ROOT / name for name in module_names]
    sources.extend(sorted((PROJECT_ROOT / "tests").glob("*.py")))
    sources.extend(PROJECT_ROOT / "network" / name for name in network_names)
    missing = [str(path) for path in sources if not path.is_file()]
    if missing:
        raise RuntimeError(f"pytest sandbox sources missing: {missing}")
    copied = []
    for source in sources:
        ...
        if source_hash != target_hash:
            raise RuntimeError(f"pytest sandbox copy hash mismatch: {source} -> {target}")
        copied.append({
            "source": str(source),
            "target": str(target),
    ...
    copy_manifest = {
        ...
        "isolated_project_cwd": str(sandbox_project),
        "selection": "production modules required by the complete existing test suite, tests/*.py, and network source XML only; outputs and large documents excluded",
        "files": copied,
    }
    (sandbox_root / "copy_manifest.json").write_text(json.dumps(copy_manifest, indent=2, sort_keys=True), encoding="utf-8")
    return sandbox_project


def run_tests() -> int:
    write_command_record()
    preserve_nonisolated_pytest_incident()
    before = real_workspace_snapshot()
    sandbox_project = build_pytest_sandbox()
    command = [
        sys.executable,
        "-m",
        "pytest",
        "-q",
        "--basetemp",
        str(AUDIT_DIR / "pytest_sandbox" / "tmp"),
        "--junitxml",
        str(AUDIT_DIR / "pytest_junit.xml"),
    ]
    test_env = dict(os.environ)
    test_env["PYTHONDONTWRITEBYTECODE"] = "1"
    result = subprocess.run(command, cwd=sandbox_project, env=test_env, text=True, capture_output=True, check=False)
    after = real_workspace_snapshot()
    tracked_unchanged = before["tracked"] == after["tracked"]
    network_unchanged = before["critical_network_xml"] == after["critical_network_xml"]
    changed_tracked = sorted(
        path for path in set(before["tracked"]) | set(after["tracked"])
        if before["tracked"].get(path) != after["tracked"].get(path)
    )
    changed_network = sorted(
        path for path in set(before["critical_network_xml"]) | set(after["critical_network_xml"])
        if before["critical_network_xml"].get(path) != after["critical_network_xml"].get(path)
    )
    guard = {
        "purpose": "prove isolated pytest does not modify the real workspace",
        "before": before,
        "after": after,
        "tracked_unchanged": tracked_unchanged,
        "critical_network_xml_unchanged": network_unchanged,
        "unchanged": tracked_unchanged and network_unchanged,
        "changed_tracked_paths": changed_tracked,
        "changed_critical_network_xml_paths": changed_network,
    }
    guard_path = AUDIT_DIR / "pytest_real_workspace_hash_guard.json"
    ...
    effective_exit_code = result.returncode if guard["unchanged"] else 2
    (AUDIT_DIR / "pytest_result.json").write_text(
        json.dumps(
            {
                "command": command,
                ...
                "copy_manifest": str(AUDIT_DIR / "pytest_sandbox" / "copy_manifest.json"),
                "real_workspace_hash_guard": str(guard_path),
                "real_workspace_unchanged": guard["unchanged"],
            },
            indent=2,
        ),
        encoding="utf-8",
    ...
    if not guard["unchanged"]:
        print(f"real workspace changed during isolated pytest: tracked={changed_tracked}, network={changed_network}", file=sys.stderr)
