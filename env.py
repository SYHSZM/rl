...
class RunFailureRecordError(RuntimeError):
    """Raised when failure artifacts cannot be persisted without losing the original failure."""

    def __init__(self, failure_context: dict[str, object], record_error: BaseException):
        super().__init__(str(failure_context["failure_message"]))
...
def run_experiment(
    ...
    try:
        ...
        context = _attach_failure_context(exc, stage, output_dir, exp_id)
        if output_dir is not None:
            try:
                _failed_run_result(output_dir, exp_id, controller_name, seed, demand, windows, stage, exc, started_at)
            except RunFailureRecordError as record_failure:
                exc.record_error = record_failure.record_error
                exc.failure_context = context
        raise
    except Exception as exc:
...
def _retryable_failure(stage: str, exc: BaseException) -> bool:
    if isinstance(exc, FrozenNetworkMismatchError):
        return False
    return _RETRYABLE_FAILURE_STAGES.get(stage, False)
...
def _failure_context(
    ...
    return {
        ...
        "retryable": _retryable_failure(stage, exc),
        "output_dir": str(output_dir.resolve()) if output_dir is not None else "",
        "attempt": _attempt_number(output_dir) if output_dir is not None else "",
...
def _attach_failure_context(
    ...
    context = _failure_context(stage, exc, output_dir, experiment_id)
    exc.failure_context = context
    return context


def _attempt_number(output_dir: Path) -> int:
    prefix = "attempt_"
    if not output_dir.name.startswith(prefix):
        raise ValueError(f"invalid attempt directory name: {output_dir.name}")
    return int(output_dir.name[len(prefix):])
...
def _write_attempt_record(
    ...
    *,
    ...
    record = {
        "schema_version": "cr03-attempt-v1",
        "experiment_id": experiment_id,
        "status": status,
        "valid": valid,
        ...
        "retryable": retryable,
        "attempt": _attempt_number(output_dir),
        "started_at": started_at,
...
def _failed_run_result(
    ...
    context = _attach_failure_context(exc, stage, output_dir, experiment_id)
    summary = _invalid_summary(experiment_id, controller_name, seed, demand, str(exc))
    try:
        write_window_csv(output_dir / "windows.csv", windows)
    except Exception:
        pass
    ...
    try:
        ...
        raise RunFailureRecordError(context, record_error) from exc
    return RunResult(experiment_id, output_dir, False, str(exc), windows, summary)
